...
import { CheckCircle2, PieChart, DollarSign, Timer, TrendingUp, Target, Users, ArrowRight } from "lucide-react"
...
const ProcessStep = ({ icon: Icon, title, description, step }) => (
  <div className="relative group snap-start rounded-lg p-4 overflow-hidden bg-white/5 backdrop-blur-lg rounded-xl p-8 md:p-12 hover:bg-white/10 transition-all duration-500">
    <div className="relative z-10">
      <div className="absolute left-0 w-16 h-16 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center transform transition-all duration-500 group-hover:scale-110 group-hover:bg-white/30">
...
export const InvestorEducationPage = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      // Calculate scroll progress for the main container
      const { scrollTop, scrollHeight, clientHeight } = scrollContainer
      const scrollableDistance = scrollHeight - clientHeight
      const currentProgress = scrollTop / scrollableDistance
  ...
  return (
    <>
      <Navbar />
      <div className="h-screen relative flex-1 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none z-0">
          <TreeBranch progress={scrollProgress} />
        </div>
        

        <div ref={mainScrollContainerRef} className="h-full overflow-y-auto snap-y snap-mandatory relative z-0">
          <main className={`relative pb-20 snap-start ${isMobile ? "" : "pt-32"}`}>
            <div className="container mx-auto px-4">
              <section className="mb-20 snap-start mt-10 text-center">
              ...
              <section className="mb-20 snap-start mt-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
                  <StatsCard
                    ...
                    delay={400}
                  />
                  <StatsCard icon={Target} value="55%" label="Nasdaq Success Stories Funded by VC" delay={0} />
                  <StatsCard icon={Users} value="70%" label="Unicorn Companies Backed by VC Funds" delay={200} />
                </div>
              </section>

              <section className="my-10 snap-start">
                <h2 className="text-3xl text-green-900 mb-5">Investment Process</h2>
                <div className={`${isMobile ? "space-y-4" : "space-y-16"}`}>
                  <ProcessStep
                    icon={CheckCircle2}
                    step={1}
